...
def test_forward():
    ...
    for i in tqdm(range(5, TEST_DIM['inputs'])):
        structure['inputs_n'] = i
        with open(PATH_NET, "w") as file:
            file.write(f"{i} 5 5 5 5 5 ")

        test_input = netStandalone.v_data_type(np.random.rand(i))
    ...
    test_input = netStandalone.v_data_type(np.random.rand(5))
    for i in tqdm(range(5, TEST_DIM['layers'])):
        aux_string += "5 "
    ...
    test_input = netStandalone.v_data_type(np.random.rand(5))
    for i in tqdm(range(5, TEST_DIM['n_per_layer'])):
    ...
    x_in = np.arange(5, TEST_DIM['inputs'])
    x_ly = np.arange(5, TEST_DIM['layers'])
    x_npl = np.arange(5, TEST_DIM['n_per_layer'])

    y = [1000*np.array(cpp_bench['inputs']), 1000*np.array(keras_bench['inputs']), 1000*np.array(fpga_bench['inputs']), 1000*np.array(gpu_bench['inputs'])]
    args = {'title': "FORWARD: INPUTS BENCH", 'x_label': "[Inputs number]", 'y_label': "[ms]", 'label': ["Cpp", "Keras", "FPGA", "GPU"]}
    plt.multiple_plot(x_in, y, args)

    y = [1000*np.array(cpp_bench['layers']), 1000*np.array(keras_bench['layers']), 1000*np.array(fpga_bench['layers']), 1000*np.array(gpu_bench['layers'])]
    args = {'title': "FORWARD: LAYERS BENCH", 'x_label': "[Layers number]", 'y_label': "[ms]", 'label': ["Cpp", "Keras", "FPGA", "GPU"]}
    plt.multiple_plot(x_ly, y, args)

    y = [1000*np.array(cpp_bench['n_per_layer']), 1000*np.array(keras_bench['n_per_layer']), 1000*np.array(fpga_bench['n_per_layer']), 1000*np.array(gpu_bench['n_per_layer'])]
    args = {'title': "FORWARD: NEURONS PER LAYER BENCH", 'x_label': "[Neurons per layer number]", 'y_label': "[ms]", 'label': ["Cpp", "Keras", "FPGA", "GPU"]}             
...
def test_backward():
    ...
    args = {'title': "BACKWARD: INPUTS BENCH", 'x_label': "[Inputs number]", 'y_label': "[ms]", 'label': ["Cpp", "Keras", "GPU"]}
    plt.multiple_plot(x_in, y, args)

    y = [1000*np.array(cpp_bench['layers']), 1000*np.array(keras_bench['layers']), 1000*np.array(gpu_bench['layers'])]
    args = {'title': "BACKWARD: LAYERS BENCH", 'x_label': "[Inputs number]", 'y_label': "[ms]", 'label': ["Cpp", "Keras", "GPU"]}
    plt.multiple_plot(x_ly, y, args)

    y = [1000*np.array(cpp_bench['n_per_layer']), 1000*np.array(keras_bench['n_per_layer']), 1000*np.array(gpu_bench['n_per_layer'])]
    args = {'title': "BACKWARD: NEURONS PER LAYER BENCH", 'x_label': "[Inputs number]", 'y_label': "[ms]", 'label': ["Cpp", "Keras", "GPU"]}             
    plt.multiple_plot_and_wait(x_npl, y, args)
...
def cpp_forward(bench_list, handler, test_input, net_name = "_temporal_net"):
    ...
    handler.net_create("cpu_float_test", netStandalone.CPU, netStandalone.NOT_DERIVATE, netStandalone.RANDOM, net_name, file_reload = True)
...
def fpga_forward(bench_list, handler, test_input,net_name = "_temporal_net"):
    ...
    handler.net_create("fpga_float_test", netStandalone.FPGA, netStandalone.NOT_DERIVATE, netStandalone.NOT_RANDOM, net_name+"_with_params", file_reload = True)
...
def gpu_forward(bench_list, handler, test_input,net_name = "_temporal_net"):
    ...
    handler.net_create("gpu_float_test", netStandalone.GPU, netStandalone.NOT_DERIVATE, netStandalone.NOT_RANDOM, net_name+"_with_params", file_reload = True)
...
def keras_forward(bench_list, structure):
    ...
    keras_model = keras.create_keras_net(structure['inputs_n'], structure['layers_n'], structure['neurons_per_layer'])
...
def cpp_backward(bench_list, handler, net_name = "_temporal_net", set_name = "_temporal_data"):
    ...
    handler.net_create("cpu_float_test", netStandalone.CPU, netStandalone.DERIVATE, netStandalone.RANDOM, net_name, file_reload = True)
    handler.set_active_net("cpu_float_test")
    handler.active_net_init_gradient(set_name, file_reload = True)
    tic = time.perf_counter()
    handler.active_net_launch_gradient(ITERATIONS, error_threshold=0.0001, multiplier=1.01)
...
def gpu_backward(bench_list, handler, net_name = "_temporal_net", set_name = "_temporal_data"):
    ...
    handler.net_create("gpu_float_test", netStandalone.GPU, netStandalone.DERIVATE, netStandalone.RANDOM, net_name, file_reload = True)
    handler.set_active_net("gpu_float_test")
    handler.active_net_init_gradient(set_name, file_reload = True)
    tic = time.perf_counter()
    handler.active_net_launch_gradient(ITERATIONS, error_threshold=0.0001, multiplier=1.01)
...
def validate_forward(res_cpu, res_fpga, error_msg, error_index):
    ...
    if np.sum(rnd_cpu == rnd_fpga) != len(res_fpga):
        ...
        if error_distance < 1:
            logger.log("FPGA FORWARD FAIL: "+ error_msg + str(error_index) + "  ERROR_DISTANCE: " + str(error_distance), "WARNING")
        else:
            logger.log("FPGA FORWARD FAIL: "+ error_msg + str(error_index) + "  ERROR_DISTANCE: " + str(error_distance), "ERROR")
